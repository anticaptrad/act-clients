...
public enum ActClientError: Error {
    case invalidURL
    case http(status: Int, body: Data)
}

public struct ActClient: Sendable {
    ...
    public func request(method: String, path: String, body: Data? = nil) async throws -> Data {
        guard let url = URL(string: path.trimmingCharacters(in: CharacterSet(charactersIn: "/")), relativeTo: baseURL.appendingPathComponent("/")) else {
            throw ActClientError.invalidURL
        }
        ...
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
            throw ActClientError.http(status: (response as? HTTPURLResponse)?.statusCode ?? -1, body: data)
        }
        return data
    }
