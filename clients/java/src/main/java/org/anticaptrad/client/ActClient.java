package org.anticaptrad.client;
...
public final class ActClient {
    private final String baseUrl;
    private final String token;
    private final HttpClient httpClient;

    public ActClient(String baseUrl, String token) {
    ...
    public String request(String method, String path, String jsonBody) throws IOException, InterruptedException {
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            throw new IOException("Anticaptrad API returned HTTP " + response.statusCode() + ": " + response.body());
        }
